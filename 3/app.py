"""Main Streamlit application for document processing and chat (no retrieval)."""
...
def initialize_session_state():
    ...
    # Store all processed chunks in-session keyed by file name
                    ...
                    try:
                        ...
                        with tempfile.TemporaryDirectory() as tmp_dir:
                            ...
                            for uploaded_file in uploaded_files:
                                ...
                                # Store all chunks per-file in session
                                if file_name not in per_file_map:
                                    per_file_map[file_name] = []
                                per_file_map[file_name].extend(chunks)
                        ...
                        # Save per-file chunks to session (in-memory only)
                        for fname, docs in per_file_map.items():
                            st.session_state.file_chunks[fname] = docs
        ...
        if not files:   
            ...
            for item in files:
                ...
                with st.expander(f"{file_name} ({chunk_count} chunks)"):
                    ...
                    if show_all:
                        all_chunks = st.session_state.file_chunks.get(file_name, [])
                        st.caption(f"Displaying {len(all_chunks)} chunk(s) from memory")
...
def display_chat():
    ...
    st.title("💬 Chat")
    st.markdown("Chat with the assistant. Document uploads are for processing previews only - processing and chat are not connected.")
    ...
    if prompt := st.chat_input("Ask a question about your documents..."):
        # Chat works without retrieval; allow even if no documents
        ...
        # Get response from OpenRouter (no retrieval)
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                try:
                    completion = st.session_state.openrouter_client.chat.completions.create(
                        model=config.LLM_MODEL,
                        messages=[
                            {"role": "system", "content": "You are a helpful assistant. The user may upload documents, but you do not have access to their contents. Answer based only on the user's messages."},
                    ...
                    answer = f"Sorry, I couldn't generate a response: {str(e)}"
